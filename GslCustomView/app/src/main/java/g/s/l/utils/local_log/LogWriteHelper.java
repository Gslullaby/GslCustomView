...
public class LogWriteHelper {
    ...
    public synchronized static void writeLogToFile(String fileName, String msg, String dirPath)
            throws IOException {
    ...
    private static boolean cutLogToHalf(File old) {
        ...
        try {
            ...
            bfdTempWriter = new BufferedWriter(tempWriter);//use better buf?
     ...
     * @param dirPath full dir path
     */
    public static void deleteExpireLogFiles(String dirPath) {
        ...
        if (!dirFile.exists() && !dirFile.mkdirs()) {
            ...
            for (File f : dirFile.listFiles()) {
                if (!remainFileNames.contains(f.getName()) && f.exists() && f.isFile()) {
                    Log.d(TAG, "delete logFile: " + f.getAbsolutePath() + ", result=" + f.delete());
                }
            }
        }
